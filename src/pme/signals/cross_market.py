...
class CostModel:
    ...
    def cost_per_contract(self, buy_venue: Venue, hedge_venue: Venue, buy_price: float, hedge_no_price: float) -> float:
...
def detect_cross_venue(
    ...
    now = utcnow()
    filtered = [q for q in quotes if max_quote_age_seconds is None or (now - q.timestamp).total_seconds() <= max_quote_age_seconds]
    ...
    for mapping in mappings:
        ...
        if q is None or q.bid is None or q.ask is None:
            continue
        grouped.setdefault((mapping.canonical_event_id, mapping.canonical_outcome), []).append((mapping, q))

    opportunities: list[Opportunity] = []
    for (event_id, outcome), rows in grouped.items():
        for (ma, qa), (mb, qb) in combinations(rows, 2):
            if qa.venue == qb.venue:
                continue
            if max_pair_skew_seconds is not None and abs((qa.timestamp - qb.timestamp).total_seconds()) > max_pair_skew_seconds:
                continue
            ...
            for buy, hedge in directions:
                ...
                cost = cost_model.cost_per_contract(
                    buy.venue, hedge.venue, buy.ask, hedge_no_price
                )
                net = gross_edge - cost
                if net < min_net_edge:
                    continue
