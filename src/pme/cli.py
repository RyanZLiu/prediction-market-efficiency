...
def suggest_matches_cmd(
    ...
    for s in suggestions:
        table.add_row(f"{s.score:.1f}", s.kalshi_market_id, s.polymarket_market_id, s.kalshi_question[:55], s.polymarket_question[:55])
    console.print(table)
...
def export_suggestions(
    ...
    db = _db(); markets = db.markets(); db.close()
    suggestions = suggest_matches(markets, threshold=threshold, limit=limit)
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["score", "kalshi_market_id", "polymarket_market_id", "polymarket_token_id", "kalshi_question", "polymarket_question"])
        for s in suggestions:
            writer.writerow([s.score, s.kalshi_market_id, s.polymarket_market_id, s.polymarket_token_id, s.kalshi_question, s.polymarket_question])
...
def import_mappings(path: Path) -> None:
    ...
    db = _db(); count = db.upsert_mappings(mappings); db.close()
    ...
    async with KalshiClient() as kalshi, PolymarketClient() as poly:
        ...
        async def fetch(m):
            async with semaphore:
                ...
                except Exception as exc:  # one bad market should not kill an entire collection cycle
                    console.print(f"[yellow]collect warning[/yellow] {m.venue}:{m.market_id}: {exc}")
                return None
...
def scan(
    ...
    max_pair_skew: float = typer.Option(90.0, help="Reject venue quotes farther apart than this many seconds."),
    max_quote_age: float = typer.Option(180.0, help="Reject latest quotes older than this many seconds."),
) -> None:
    db = _db()
    quotes = db.latest_quotes()
    mappings = db.mappings(verified_only=True)
    cost = CostModel(
        fee_bps={Venue.KALSHI: settings.kalshi_fee_bps, Venue.POLYMARKET: settings.polymarket_fee_bps},
        slippage_bps=settings.slippage_bps,
    )
    opportunities = detect_cross_venue(quotes, mappings, cost_model=cost, min_net_edge=min_edge, max_pair_skew_seconds=max_pair_skew, max_quote_age_seconds=max_quote_age)
    db.insert_opportunities(opportunities)
    db.close()
    table = Table("Event", "Buy", "Hedge", "Gross", "Cost", "Net", "Size")
    for o in opportunities[:30]:
        table.add_row(o.canonical_event_id, o.buy_venue.value, o.hedge_venue.value,
                      f"{100*o.gross_edge:.2f}%", f"{100*o.estimated_cost:.2f}%",
                      f"{100*o.net_edge:.2f}%", "?" if o.available_size is None else f"{o.available_size:.1f}")
    console.print(table)
...
def backtest(
    ...
    db = _db(); opportunities = db.opportunities()
    engine = PaperBacktester(BacktestConfig(min_net_edge=min_net_edge, max_contracts=max_contracts, cooldown_seconds=cooldown))
...
def collect_odds(
    ...
    async def run():
        async with OddsApiClient() as client:
            return await client.odds(sport_key=sport_key, regions=regions, markets=markets, bookmakers=bookmakers)
    quotes = asyncio.run(run())
    db = _db(); count = db.insert_sportsbook_quotes(quotes); db.close()
    table = Table("Book", "Market", "Outcome", "Odds", "Implied", "De-vig")
    for q in quotes[:30]:
        table.add_row(q.bookmaker, q.market_key, q.outcome, f"{q.american_odds:+.0f}", f"{100*q.implied_probability:.2f}%", f"{100*(q.fair_probability or 0):.2f}%")
    console.print(table)
...
def constraints_cmd(path: Path) -> None:
    ...
    constraints = [ProbabilityConstraint(
    ...
    for name, observed in prices.items():
        table.add_row(name, f"{observed:.4f}", f"{repaired[name]:.4f}", f"{repaired[name]-observed:+.4f}")
    console.print(table)
...
def demo() -> None:
    ...
    ts = utcnow()
    mappings = [
        MarketMapping(canonical_event_id="demo_event", canonical_outcome="YES", venue=Venue.KALSHI, market_id="DEMO-K", question="Demo proposition", verified=True),
        MarketMapping(canonical_event_id="demo_event", canonical_outcome="YES", venue=Venue.POLYMARKET, market_id="DEMO-P", token_id="DEMO-TOKEN", question="Demo proposition", verified=True),
    ]
    db = _db(); db.upsert_mappings(mappings)
    for i in range(12):
        t = ts + timedelta(seconds=60*i)
        k = Quote(timestamp=t, venue=Venue.KALSHI, market_id="DEMO-K", bid=.50+i*.001, ask=.52+i*.001, bid_size=100, ask_size=100)
        p = Quote(timestamp=t, venue=Venue.POLYMARKET, market_id="DEMO-P", token_id="DEMO-TOKEN", bid=.57-i*.001, ask=.59-i*.001, bid_size=80, ask_size=80)
        db.insert_quotes([k, p])
        opps = detect_cross_venue([k, p], mappings, CostModel(slippage_bps=5), min_net_edge=0, max_quote_age_seconds=None)
        db.insert_opportunities(opps)
    engine = PaperBacktester(BacktestConfig(min_net_edge=.005, max_contracts=25, cooldown_seconds=120))
...
def stream_polymarket_cmd(seconds: int = typer.Option(120, min=1)) -> None:
    db = _db(); mappings = [m for m in db.mappings(True) if m.venue == Venue.POLYMARKET and m.token_id]
...
def stream_kalshi_cmd(seconds: int = typer.Option(120, min=1)) -> None:
    db = _db(); tickers = [m.market_id for m in db.mappings(True) if m.venue == Venue.KALSHI]
    async def handler(q):
        db.insert_quotes([q])
    try:
...
def lead_lag(event_id: str, max_lag: int = typer.Option(10)) -> None:
    db = _db(); rows = db.quote_series(event_id); db.close()
...
def convergence(
    ...
    for e in episodes:
        table.add_row(str(e.start), str(e.end), f"{e.duration_seconds:.1f}", f"{100*e.max_gap:.2f}%")
    console.print(table)
