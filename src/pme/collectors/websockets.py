...
async def stream_polymarket(
    ...
    async with websockets.connect(endpoint, ping_interval=None) as ws:
        await ws.send(
            json.dumps({"assets_ids": token_ids, "type": "market", "custom_feature_enabled": True})
        )
            ...
            while deadline is None or time.monotonic() < deadline:
                ...
                for msg in messages:
                    ...
                    if event_type == "book":
                        ...
                        bid_size = next(
                            (float(x["size"]) for x in bids if float(x["price"]) == best_bid), None
                        )
                        ask_size = next(
                            (float(x["size"]) for x in asks if float(x["price"]) == best_ask), None
                        )
                        latest[token] = {
                            "bid": best_bid,
                            "ask": best_ask,
                        ...
                        await handler(
                            Quote(
                                timestamp=_ms_to_dt(msg.get("timestamp")),
