...
def reliability_bins(probabilities: list[float], outcomes: list[int], bins: int = 10) -> list[dict[str, float]]:
    if len(probabilities) != len(outcomes):
        raise ValueError("probabilities and outcomes must have equal length")
    result: list[dict[str, float]] = []
    for i in range(bins):
        low, high = i / bins, (i + 1) / bins
        rows = [(p, y) for p, y in zip(probabilities, outcomes) if low <= p < high or (i == bins - 1 and p == 1)]
        if not rows:
            continue
        result.append({
            "bin_low": low,
