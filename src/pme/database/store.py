...
class Database:
    ...
    def upsert_markets(self, markets: Iterable[Market]) -> int:
        ...
        for m in rows:
            self.conn.execute(
                ...
                """,
                [
                    m.venue.value, m.market_id, m.event_id, m.question, m.outcome, m.token_id,
                    m.category, m.market_type.value, m.start_time, m.close_time, m.active, m.volume,
                    m.liquidity, json.dumps(m.metadata, default=str), m.updated_at,
                ],
            )
        return len(rows)
    ...
    def insert_quotes(self, quotes: Iterable[Quote]) -> int:
        ...
        self.conn.executemany(
            ...
            [[
                q.timestamp, q.venue.value, q.market_id, q.token_id, q.outcome, q.bid, q.ask,
                q.bid_size, q.ask_size, q.last, q.volume, json.dumps(q.raw, default=str),
            ] for q in rows],
        )
        return len(rows)
    ...
    def insert_sportsbook_quotes(self, quotes: Iterable[SportsbookQuote]) -> int:
        ...
        self.conn.executemany(
            ...
            [[q.timestamp, q.sport_key, q.event_id, q.commence_time, q.home_team, q.away_team,
              q.bookmaker, q.market_key, q.outcome, q.american_odds, q.point,
              q.implied_probability, q.fair_probability, json.dumps(q.metadata, default=str)] for q in rows],
        )
        return len(rows)
    ...
    def upsert_mappings(self, mappings: Iterable[MarketMapping]) -> int:
        ...
        for m in rows:
            self.conn.execute(
                ...
                [m.canonical_event_id, m.canonical_outcome, m.venue.value, m.market_id,
                 m.token_id, m.question, m.confidence, m.verified],
            )
        return len(rows)
    ...
    def insert_opportunities(self, opportunities: Iterable[Opportunity]) -> int:
        ...
        self.conn.executemany(
            ...
            [[
                o.timestamp, o.canonical_event_id, o.canonical_outcome, o.buy_venue.value,
                o.buy_market_id, o.buy_price, o.buy_size, o.hedge_venue.value, o.hedge_market_id,
                o.hedge_yes_bid, o.hedge_no_price, o.hedge_size, o.gross_edge, o.estimated_cost,
                o.net_edge, o.available_size, json.dumps(o.metadata, default=str),
            ] for o in rows],
        )
        return len(rows)
    ...
    def insert_paper_trades(self, trades: Iterable[PaperTrade]) -> int:
        ...
        self.conn.executemany(
            ...
            [[
                t.timestamp, t.canonical_event_id, t.canonical_outcome, t.quantity,
                t.gross_edge, t.net_edge, t.locked_cost_per_contract, t.pnl,
                t.buy_venue.value, t.hedge_venue.value,
            ] for t in rows],
        )
        return len(rows)
    ...
    def mappings(self, verified_only: bool = True) -> list[MarketMapping]:
        ...
        return [MarketMapping(
            canonical_event_id=r[0], canonical_outcome=r[1], venue=Venue(r[2]), market_id=r[3],
            token_id=r[4], question=r[5], confidence=r[6], verified=r[7],
        ) for r in rows]
    ...
    def markets(self, venue: Venue | None = None) -> list[Market]:
        ...
        return [Market(
            venue=Venue(r[0]), market_id=r[1], event_id=r[2], question=r[3], outcome=r[4],
            token_id=r[5], category=r[6], market_type=r[7], start_time=r[8], close_time=r[9],
            active=r[10], volume=r[11], liquidity=r[12], metadata=json.loads(r[13] or "{}"),
            updated_at=r[14],
        ) for r in rows]
    ...
    def latest_quotes(self) -> list[Quote]:
        ...
        return [Quote(
            timestamp=r[0], venue=Venue(r[1]), market_id=r[2], token_id=r[3], outcome=r[4],
            bid=r[5], ask=r[6], bid_size=r[7], ask_size=r[8], last=r[9], volume=r[10],
    ...
    def opportunities(self, limit: int | None = None) -> list[Opportunity]:
        ...
        rows = self.conn.execute(sql).fetchall()
        return [Opportunity(
            timestamp=r[0], canonical_event_id=r[1], canonical_outcome=r[2], buy_venue=Venue(r[3]),
            buy_market_id=r[4], buy_price=r[5], buy_size=r[6], hedge_venue=Venue(r[7]),
            hedge_market_id=r[8], hedge_yes_bid=r[9], hedge_no_price=r[10], hedge_size=r[11],
            gross_edge=r[12], estimated_cost=r[13], net_edge=r[14], available_size=r[15],
